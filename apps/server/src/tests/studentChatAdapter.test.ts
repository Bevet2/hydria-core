...
test("student chat adapter routes stable biographies through the Mistral factual writing route", async () => {
  ...
  assert.equal(result.runtimeBudget?.maxOutputTokens, 88);
  assert.equal(numPredict, 88);
...
test("student chat prompt compacts stable factual biographies", () => {
  ...
  assert.match(prompt, /25-45 words/i);
  assert.match(prompt, /exactly one short key point/i);
